...
- 매출·마케팅·이벤트·교재·검색 트렌드 데이터를 하나의 대시보드에서 조회·분석할 수 있도록 구현
- 사업부·기간·상품·분석 영역별 조회 조건 구성
- 통합 데이터를 기반으로 전년 대비 변화와 주요 성과 지표를 분석하는 흐름 구축

### 3. AI Analysis
- 대시보드에서 조회한 데이터를 분석 목적에 따라 선택적으로 AI 컨텍스트로 구성
- Gemini 기반 AI 성과 분석 리포트 생성
- 매출·GA·이벤트·교재·검색 트렌드 등 여러 분석 영역을 하나의 리포트로 종합

### 4. Report Knowledge Base
- 생성된 AI 리포트와 담당자 검토 의견을 함께 저장
- AI 분석에 사용된 원본 데이터 요약 및 스냅샷 저장
- 리포트를 섹션 단위 검색 청크로 구성하고 embedding 생성
- BigQuery `VECTOR_SEARCH` 기반 RAG 검색 구조 구현

### 5. Comparison & Reuse
- 현재 리포트와 과거 보관 리포트를 비교하여 개선·악화 항목과 반복 패턴 분석
- 과거 담당자의 의견까지 비교 분석에 활용
- 축적된 AI 분석 결과를 이후 검색·비교·의사결정에 재활용할 수 있도록 구성

> 🔒 실무 프로젝트로 실제 업무 데이터와 소스 코드는 보안상 공개하지 않습니다.  
> 대신 공개 가능한 범위에서 데이터 흐름, 시스템 구조, 분석 로직 및 구현 범위를 정리했습니다.
> 
## 기술 아키텍처
